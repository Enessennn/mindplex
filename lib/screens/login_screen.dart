...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _girisYap() async {
    ...
    try {
      ...
      // Başarılıysa MainScreen'e yönlendir
      if (mounted) {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const MainScreen()));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Giriş Hatası: ${e.toString()}"), backgroundColor: Colors.red));
    } finally {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
             // Logo niyetine kilit ikonu
            const Icon(Icons.lock_person_rounded, size: 100, color: Colors.deepPurple),
            const SizedBox(height: 20),
            const Text("Hoş Geldiniz", textAlign: TextAlign.center, style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.deepPurple)),
